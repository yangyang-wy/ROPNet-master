...
def Error_R(r1, r2):   # R误差
    '''
    Calculate isotropic rotation degree error between r1 and r2. 计算r1和r2之间的各向同性旋转度误差
    :param r1: shape=(B, 3, 3), pred
    :param r2: shape=(B, 3, 3), gt
    :return:
    '''
    r2_inv = r2.transpose(0, 2, 1)
    r1r2 = np.matmul(r2_inv, r1)                           # np.matmul两个numpy数组的矩阵相乘
...
def anisotropic_R_error(r1, r2, seq='xyz', degrees=True):  # 各向异性R误差
    '''
    Calculate mse, mae euler agnle error. 计算mse，mae-euler-agnle误差
    :param r1: shape=(B, 3, 3), pred
    :param r2: shape=(B, 3, 3), gt
    :return:
    '''
    ...
    eulers1 = np.stack(eulers1, axis=0)     # np.stack 堆叠数组
    eulers2 = np.stack(eulers2, axis=0)
    r_mse = np.mean((eulers1 - eulers2)**2, axis=-1)   # np.mean 求均值
    r_mae = np.mean(np.abs(eulers1 - eulers2), axis=-1)
    return r_mse, r_mae


def anisotropic_t_error(t1, t2):   # 各向异性t误差
    '''
    calculate translation mse and mae error. 计算平移mse和mae误差
    :param t1: shape=(B, 3)
    :param t2: shape=(B, 3)
    :return:
    '''
